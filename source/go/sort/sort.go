...
func main() {
	...
	var size int
	// var t0 time.Time = time.Now()
	var l []string = gen_array(x, y, &size)
	// var t1 time.Time = time.Now()
	imsort(l, 0, size)
	...
	// var exec_sub time.Duration = 0
	// for i := 1; i <= 10; i++ {
	// 	start := time.Now()
	// 	var l []string = gen_array(x, y, &size)
	// 	imsort(l, 0, size)
	// 	assert(verify_array(l))
	// 	exec_sub = exec_sub + time.Since(start)
	// }
	// fmt.Printf("%f,%f\n", float64(exec_time.Microseconds()) / 1000, float64(exec_sub.Microseconds()) / 1000 / 10)

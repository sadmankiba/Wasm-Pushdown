...
bool read_line(std::istream* is, SudokuBoard* board) {
	...
	char buf[len];
	if (!is->read(buf, len)) {
		std::cout << "board string is not 81 characters" << std::endl;
		return false;
	}
	for (int i = 0; i < 9; i++) {
		for (int j = 0; j < 9; j++) {
...
int main(int argc, char* argv[]) {
	// Measure time for the solve function
	auto start = std::chrono::high_resolution_clock::now();
	process(argv[1]);
	auto end = std::chrono::high_resolution_clock::now();

	// Calculate the duration in milliseconds
	auto exec_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
	...
	// Print the duration
	std::cout << exec_time << "," << (total_time.count() / 10) << std::endl;
